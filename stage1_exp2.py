...
class SiameseNoGapTranslationRegressor(nn.Module):
    def __init__(self) -> None:
        super().__init__()
...
def run_epoch(
    loader: DataLoader,
    model: SiameseNoGapTranslationRegressor,
    optimizer: torch.optim.Optimizer | None,
    device: str,
) -> dict[str, float]:
    training = optimizer is not None
    model.train(training)
    totals = {"loss": 0.0, "mae": 0.0, "rmse": 0.0, "mae_tx": 0.0, "mae_ty": 0.0, "mae_tz": 0.0}
    batches = 0
...
def plot_history(history: dict[str, list[float]], path: str | Path) -> None:
    ...
    axes[2].plot(epochs, history["val_mae_tx"], label="tx")
    axes[2].plot(epochs, history["val_mae_ty"], label="ty")
    axes[2].plot(epochs, history["val_mae_tz"], label="tz")
...
def train(
    ...
    model = SiameseNoGapTranslationRegressor().to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        history["val_mae_tx"].append(val_stats["mae_tx"])
        history["val_mae_ty"].append(val_stats["mae_ty"])
        history["val_mae_tz"].append(val_stats["mae_tz"])
...
def evaluate(
    ...
    metrics = {
        ...
        "mae_m": mae,
        "rmse_m": rmse,
        "mae_tx_m": float(axis_mae[0]),
        "mae_ty_m": float(axis_mae[1]),
        "mae_tz_m": float(axis_mae[2]),
    }
...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Single-file Experiment 2 baseline for 3D camera translation regression."
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    train_parser = subparsers.add_parser("train", help="Train the Experiment 2 model.")
